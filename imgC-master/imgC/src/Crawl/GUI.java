...
public class GUI extends JFrame {
	...
	GUI() {
		...
		ActionListener listener = e -> {
			...
			String UUU ="http://www.etorrent.co.kr/bbs/board.php?bo_table=star&wr_id=";
			int sta = 1276648;
			int end = 1276654;
			...
			try {
//				Document doc = Jsoup.connect(url.getText()).userAgent("Mozilla").get();
				Document doc = Jsoup.connect(UUU+Integer.toString(i)).get();
				
				
				Element element = doc.select(css.getText()).get(0);
				
				Element element2 = doc.select("td.mw_basic_view_subject").get(0);
				
				Elements img = element.select("img");
				
				Elements FN = element.select("h1");
				
//				System.out.println("img:   "+img);
//				System.out.println("FN:    "+element2.text());
...
//					if (url.getText().contains("www.etorrent.co.kr")) {
					if ((UUU+Integer.toString(i)).contains("www.etorrent.co.kr")) {

						url2 = url2.replace("../", "http://www.etorrent.co.kr/");
					}
...
//							saveLocation.getText() + "\\" + Name.getText() + "_" + fileNum + format);
							saveLocation.getText() + "\\" + element2.text() + "_" + fileNum + format);
